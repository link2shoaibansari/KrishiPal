...
#define FIREBASE_HOST "https://krishipal-2b4c5-default-rtdb.asia-southeast1.firebasedatabase.app/"
#define FIREBASE_AUTH "AIzaSyC_VZtOLxpG10YhAjCPOhZ_JH_oWoe7H8E"
...
void updateFirebase() {
  String timestamp = Firebase.getTimestamp(firebaseData);
  ...
  json.set("light", lightOn);
  json.set("motion", motionAlert);
  ...
  Firebase.setFloat(firebaseData, "/latest/temperature", tempC);
  Firebase.setFloat(firebaseData, "/latest/humidity", humidity);
  Firebase.setInt(firebaseData, "/latest/moisture", moisturePercent);
}
...
void handleRoot() {
  ...
  <head>
    ...
    <title>KrishiPal - Plant Monitoring System</title>
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    if (autoMode) {
      ...
      if (pumpOn && (moisturePercent >= 40 || currentMillis - pumpStartTime >= minPumpDuration)) {
        pumpOn = false;
        digitalWrite(MOTOR_IN1, LOW);
        digitalWrite(MOTOR_IN2, LOW);
    ...
    Serial.printf("Temp: %.1f°C | Hum: %.1f%% | Soil: %d%% | Pump: %s | Light: %s\n",
      tempC, humidity, moisturePercent, pumpOn ? "ON" : "OFF", lightOn ? "ON" : "OFF");
...
void updateLCD() {
  ...
  if (pirEnabled && motionAlert && millis() < motionDisplayUntil) {
